...
DisplayWindow::DisplayWindow(FileSystem *fs, PageMemoryPool *memoryPool) :fs(fs), memoryPool(memoryPool)
{
...
void DisplayWindow::refreshWindow()
{
	...
	for (int i = 0; i < processList.size() && i < 18; i++)
	{
		...
		SDL_Surface* textLine = TTF_RenderText_Solid(font, tmpLIst.c_str(), SDL_Color{ 0,0,0 });
	...
	PageMemStatus memStatus = memoryPool->getMemStatus();
	string tmpStr = "nPhysicalPage: " + to_string(memStatus.nPhysicalPage);
	SDL_Surface* textLine = TTF_RenderText_Solid(font, tmpStr.c_str(), SDL_Color{ 0,0,0 });
	...
	{
		SDL_Surface* textLine = TTF_RenderText_Solid(font, fileList[i].c_str(), SDL_Color{ 0,0,0 });
