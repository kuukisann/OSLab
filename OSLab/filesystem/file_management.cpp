...
//解析绝对路径，从左到右找到当前目录/文件的dir* 
dir* analyse_Path(string path){  
	int pos, i;
	string father, son;
...
//创建文件 
iNode* Create_File(string pathname, unsigned short f_type){ //参数在Fill_in_iNode上还有一些，讨论之后再加 
	string path, f_name;
	dir* item;
	if (pathname[0] != '/') {//相对路径，依据current_dir
		...
		item = analyse_Path(path); //解析路径，得到其所在目录文件的dir数组 
	}
	...
	if (f_type == 0) { //f_type为0时说明创建目录文件
		...
		//当前情况下一个目录占四个block
		dir *tmp_buf = (dir*)malloc(sizeof(temp_dir));
		memcpy(tmp_buf, temp_dir, sizeof(temp_dir));
		for (int k = 0; k < FBLK_NUM; k++) {
			...
			fwrite((tmp_buf + k * BLOCK_SIZE), BLOCK_SIZE, 1, disk_p);
		}
		fclose(disk_p);
	}
	iNode_table[(*(item+i)).iNode_no] = newiNode; 
	
	//(4)写入磁盘，iNode_table更新，item所指的目录进行更新
	FILE *disk_p = fopen(DEV_NAME, "rb+");
...
os_file* Open_File(string f_name){
	...
	if(f_name[0] == '/'){//绝对路径 
		...
		dir* current= analyse_Path(f_name.substr(0,rpos)); //得到该文件所在目录的dir*数组 
		if (current == NULL)
			return NULL;
